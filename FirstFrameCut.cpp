...
// 函数声明
void save_frame(AVFrame *frame, int width, int height, const std::string &output_folder);
...
int main(int argc, char *argv[]) {
    ...
    while (av_read_frame(format_ctx, &packet) >= 0) {
        if (packet.stream_index == video_stream_index) {
            ...
            if (ret == 0) {
                // 保存第一帧
                save_frame(frame, codec_ctx->width, codec_ctx->height, output_folder);
                break; // 保存第一帧后立即退出循环
            }
        }
...
void save_frame(AVFrame *frame, int width, int height, const std::string &output_folder) {
    ...
    filename << output_folder << "/first_frame.jpg";
    ...
    std::cout << "Saved first frame" << std::endl;
}
